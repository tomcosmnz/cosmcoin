...
#if defined(NDEBUG)
# error "Cosmcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x519b4a552751be1dc46c7603d2f9fae51a9689eaf974a02847d3181c94fdaef6");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Cosmcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Cosmcoin Signed Message:\n";
...
{
    // Cosmcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Cosmcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Cosmcoin: 1 days
static const int64 nTargetSpacing = 5 * 60; // Cosmcoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Cosmcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Cosmcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xcd57b4ae095805cf26bdb086d3ec7d406661918d9810c0c133bb9c95c669e6df");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Nearly 400kg of cocaine was found inside the Russian embassy in Buenos Aires";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04aa3a0c889915987e329f26f322ffb33f25245e9e681285843277b6b26ee6489940afca6d57fd6f047af1411e79ddfc7bb84e456afe9258e12247e83c72e5ef57") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1519379457;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084710389;

        if (fTestNet)
        {
            block.nTime    = 1519379443;
            block.nNonce   = 385426171;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xc8cb8eea69e8c26db395cf1a774d6b73799e33deec3bcc01d837ebe373d38e64"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc1, 0xb5, 0xdc }; // Cosmcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CosmcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CosmcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CosmcoinMiner(CWallet *pwallet)
{
    printf("CosmcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cosmcoin-miner");
    ...
    try { loop {
        ...
        printf("Running CosmcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CosmcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CosmcoinMiner, pwallet));
}
